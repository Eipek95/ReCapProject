...
@Component({
  selector: 'app-car-test-update',
  templateUrl: './car-test-update.component.html',
  styleUrls: ['./car-test-update.component.css'],
})
export class CarTestUpdateComponent implements OnInit {
  currentCar: Car;
  brands: Brand[];
  colors: Color[];
  years: number[] = [];
  ...
  constructor(
    private carUpdateModal: MatDialogRef<CarTestUpdateComponent>,
  ...
  update() {
    if (!this.carUpdateForm.valid) {
      ...
      let deletedImages = this.currentCar.carImages.filter(
        (image) => image.id != -1 && this.carImages.indexOf(image) == -1
      );
      let carInfoChanged: boolean = this.checkIfCarObjectChanged(
        carUpdateModel,
        this.currentCar
      );
      let carImagesChanged: boolean = this.checkIfCarImagesChanged(
        this.uploadFiles,
        deletedImages
      );
      if (!carInfoChanged && !carImagesChanged) {
        ...
        if (this.carImages.length + this.uploadFiles.length > 5) {
          ...
          this.updateCarImages(deletedImages, this.uploadFiles).then(
            (updateCarImageResult) => {
              this.carService
                .update(carUpdateModel)
                .subscribe((updateSuccess) => {
                  if (updateCarImageResult.length > 0) {
                    this.toastrService.warning(
                      'Araç başarıyla güncellendi fakat bazı resimler güncellenemedi. ' +
                        updateCarImageResult,
                      'Araç kısmen güncellendi'
                    );
  ...
  private checkIfCarImagesChanged(
  ...
  private checkIfCarObjectChanged(newCarObject: any, oldCarObject: Car) {
    return !(
      newCarObject.brandId == oldCarObject.brandId &&
  ...
  private updateCarImages(
    deletedImages: Carimage[],
    uploadFiles: UploadFile[]
  ): Promise<string> {
    return new Promise<string>((methodResolve) => {
      if (this.carImages.length < this.currentCar.carImages.length) {
        //If the car image was deleted
        deletedImages = this.currentCar.carImages.filter(
          (image) => image.id != -1 && this.carImages.indexOf(image) == -1
        );
      }
      ...
      let deletePromise = new Promise<void>((deletePromiseResolve) => {
        this.deleteAllSelectedImagesFromServer(deletedImages).then(
          (deleteResolve) => {
  ...
  private deleteAllSelectedImagesFromServer(
    deletedImages: Carimage[]
  ): Promise<Carimage[]> {
      ...
      if (deletedImages.length > 0) {
        ...
        const allDelets = new Promise<void>(async (resolveAllDelets) => {
          ...
          for (const image of deletedImages) {
            await this.deleteImageFromServer(image)
              .then(
                (deleteSuccessFile) => {},
                (deleteFailFile) => {
  ...
  private deleteImageFromServer(deletedImage: Carimage): Promise<Carimage> {
    return new Promise<Carimage>((resolve, reject) => {
      this.carImageService.deleteImage(deletedImage).subscribe(
        (deleteSuccess) => {
  ...
  private uploadAllImagesToServer(
    uploadFiles: UploadFile[]
  ): Promise<UploadFile[]> {
  ...
  private uploadImageToServer(uploadFile: UploadFile): Promise<UploadFile> {
    return new Promise<UploadFile>((result) => {
      this.carImageService
        .uploadImage(uploadFile.file, this.currentCar.id)
        .subscribe(
          (uploadSuccess) => {
    ...
    if (imageList.length !== 0) {
      if (this.carImages.length + this.uploadFiles.length < 5) {
        for (let i = 0; i < imageList.length; i++) {
          ...
          uploadFile.uploadStatus = false;
          let preselectedFile = this.uploadFiles.find(
            (uploadFile) => uploadFile.file.name === image.name
          );
          if (preselectedFile === undefined) {
            ...
            this.toastrService.warning(
              'Bu resmi daha önce listeye eklediniz',
              'Zaten listede'
            );
          }
        }
        ...
        this.toastrService.error(
          'En fazla 5 resim ekleyebilirsiniz',
          'Resim eklenemiyor'
        );
      }
    }
  }
  private addCarImageToUploadImagesPath(image: any): Promise<boolean> {
    //Source: https://www.talkingdotnet.com/show-image-preview-before-uploading-using-angular-7/
  ...
  deleteImageFromUploadFiles(uploadFile: UploadFile) {
    this.uploadImagesPaths.splice(
      this.uploadImagesPaths.indexOf(uploadFile),
      1
    );
  ...
  private getCarImages() {
    this.currentCar.carImages.forEach((image) => {
      if (image.id != -1) {
        //Do not add the default vehicle image to the list.
